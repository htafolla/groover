...
export function variantFromTraits(hat: Hat, colorway: Colorway): number {
  const hatIndex = HATS.indexOf(hat);
  const colorwayIndex = COLORWAYS.indexOf(colorway);
  if (hatIndex < 0 || colorwayIndex < 0) {
    throw new Error('unknown hat or colorway');
  }
  return (hatIndex << 2) | colorwayIndex;
}
