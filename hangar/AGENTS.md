...
- **Pay:** unpaid GET → 402 (no wallet). To settle: install OWS ([docs.openwallet.sh](https://docs.openwallet.sh)), `ows wallet create --name agent-treasury-1`, send **USDC on Base** from another wallet or exchange (not Ethereum mainnet) to the `eip155:8453` address, then `sign_x402` `approved=true` — or `CLEARING_SIGNER=awal`. Hosted `/sign` is 410. Reuse `paymentId`. Never twice. No Groover API key to pay.
