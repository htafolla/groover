...
- **Pay:** unpaid GET → 402 (no wallet). To settle: local OWS wallet in `~/.ows`, funded with USDC on Base, `sign_x402` `approved=true` — or `CLEARING_SIGNER=awal`. Hosted `/sign` is 410. Reuse `paymentId`. Never twice. No Groover API key to pay.
